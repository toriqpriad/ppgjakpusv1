...
<?php 
	...
	$skripdetail="SELECT * FROM data_caberawit inner join data_tpq on data_caberawit.tpq_desa=data_tpq.id_tpq where id_cbr='".$_GET['detail']."' AND tpq_desa='".$tpq_user."'";
	...
	echo '
		...
		<div class="col-md-3">
				<br>
				<img src="../images/foto_cbr/'.$selectdetail2->foto_cbr.'" class="img-responsive img-thumbnail" style="width:450px; height:330px;">	
			</div>
		<div class="col-md-9">
 ...
 <div class="panel-footer">
	...
	<a href="data-caberawit.php?hapus=<?=$selectdetail2->id_cbr;?>" onclick="return confirm('Yakin menghapus data ini?')" class="btn btn-danger"><span class="glyphicon glyphicon-trash" aria-hidden="true" title="Hapus"></span> Hapus</a>
	<?php
	echo '
	</div>
	<div class="col-md-6">
	<p class="text-right">
	<a href="export_to_xls_tpq.php?peng&&detail='.$selectdetail2->id_cbr.'" class="btn btn-warning"><i class=""fa fa-file-excel-o"" aria-hidden="true" title="Download sebagai Excel"></i> Download sebagai Excel (.xls)</a>
	<a href="export_to_word_tpq.php?peng&&detail='.$selectdetail2->id_cbr.'" class="btn btn-info"><i class=""fa fa-file-word-o"" aria-hidden="true" title="Download sebagai Dokumen"></i> Download sebagai Dokumen (.docx)</a>
	</p>
	</div>
...
<script type="text/javascript">
	  ...
	  if (isset($_GET['edit']))
	{
		...
		$skripedit="SELECT * FROM data_caberawit inner join data_tpq on data_caberawit.tpq_desa=data_tpq.id_tpq where id_cbr='".$_GET['edit']."' AND data_caberawit.tpq_desa='$tpq_user' ";
		$selectedit = mysql_query($skripedit) or die(mysql_error());
		$selectedit2 = mysql_fetch_object($selectedit);
		$id=$selectedit2->id_cbr;
		echo '
   ...
   <ul id="tabs" class="nav nav-tabs" data-tabs="tabs">
    ...
    <div id="my-tab-content" class="tab-content">
		...
		<input type="text" name="tx_nm_sklh" class="form-control" value="'.$selectedit2->nm_sklh.'" name="tx_nm_sklh">
        </div>
		...
		<br>
			<img src="../images/foto_cbr/'.$selectedit2->foto_cbr.'" id="gambar_nodin" width="400" alt="Preview Gambar" class="img-thumbnail img-responsive"/>
	...
	';
		...
		if (empty($foto))
		{ 
		$isi="Update data_caberawit set id_cbr='".$id."' , nm_cbr='".$nm."' ,j_klmn='".$klmn."',tmp_lhr='".$tmp_lhr."', tgl_lhr='".$tgl_lhr."',pdkn='".$pdkn."', kls='".$kls."',nm_sklh='".$nm_sklh."', hobi='".$hobi."',  bapak='".$bpk."', ibu='".$ibu."',kontak_cbr='".$kontak."', alamat_cbr='".$almt."'  WHERE id_cbr = '".$id."'";
		}
		elseif (!empty($foto))
		{ $isi="Update data_caberawit set id_cbr='".$id."' , nm_cbr='".$nm."' ,j_klmn='".$klmn."',tmp_lhr='".$tmp_lhr."', tgl_lhr='".$tgl_lhr."',pdkn='".$pdkn."', kls='".$kls."',nm_sklh='".$nm_sklh."', hobi='".$hobi."',  bapak='".$bpk."', ibu='".$ibu."',kontak_cbr='".$kontak."', alamat_cbr='".$almt."',foto_cbr='".$foto."'  WHERE id_cbr = '".$id."'";
		...
		echo "<script type='text/javascript'>alert('Data berhasil diubah');window.location.href='detail-caberawit.php?detail=$id';</script>";
