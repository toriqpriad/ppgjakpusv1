...
<html lang="en">
    ...
    <div class="main-content">
        <div class="panel panel-default">
            ...
            function detail() {
                if (isset($_GET['detail'])) {
                    ...
                    $skripdetail = "SELECT * FROM data_praremaja inner join data_tpq on data_praremaja.tpq_desa=data_tpq.id_tpq where id_prmj='" . $_GET['detail'] . "' ";
        ...
        <li class="active"><a href="#detail_tpq" data-toggle="tab">Detail Praremaja</a></li>
    ...
    <div id="my-tab-content" class="tab-content">
        ...
        <input type="text" value=' . $selectdetail2->id_prmj . ' class="form-control" disabled>
        </div>
        <div class="form-group">
        <label>Nama Praremaja</label>
        <input type="text" class="form-control" value="' . $selectdetail2->nm_prmj . '" disabled>
        </div>
		<div class="form-group">
		...
		<div class="form-group">
        <label>Kontak</label>
        <input type="text" value="' . $selectdetail2->kontak_prmj . '" class="form-control" disabled>
        </div>
        <div class="form-group">
          <label>Alamat</label>
          <textarea rows="3" class="form-control" disabled>' . $selectdetail2->alamat_prmj . '</textarea>
        </div>      
      ...
      <div class="tab-pane" id="foto">
		<br>
        <img src="../images/foto_prmj/' . $selectdetail2->foto_prmj . '" class="img-responsive img-thumbnail" style="width:300px; height:300px;">	
		</div>
		...
		<br>
                ...
                <br>';
                        ?>
                        <a href="?edit=<?= $selectdetail2->id_prmj; ?>" class="btn btn-info"><span class="glyphicon glyphicon-pencil" aria-hidden="true" title="Ubah"></span> Ubah</a>
                        <a href="data-pengajar.php?hapus=<?= $selectdetail2->id_prmj; ?>" onclick="return confirm('Yakin menghapus data ini?')" class="btn btn-danger"><span class="glyphicon glyphicon-trash" aria-hidden="true" title="Hapus"></span> Hapus</a>
                ...
                if (isset($_GET['edit'])) {
                    $dirfoto = "../images/foto_prmj/";
                    require_once "../db/database.php";
                    $skripedit = "SELECT * FROM data_praremaja inner join data_tpq on data_praremaja.tpq_desa=data_tpq.id_tpq where id_prmj='" . $_GET['edit'] . "' ";
		...
		<div class="form-group">
                <label>ID Praremaja</label>
                <input type="text" value=' . $selectedit2->id_prmj . ' class="form-control" name="tx_id_prmj">
                </div>
                <div class="form-group">
                <label>Nama Praremaja</label>
                <input type="text" class="form-control" value="' . $selectedit2->nm_prmj . '" name="tx_nm_prmj">
                </div>  
		<div class="form-group">
                <label>Jenis Kelamin</label>
		...
		<div class="form-group">
                <label>Kontak</label>
                <input type="text" value="' . $selectedit2->kontak_prmj . '" class="form-control" name="tx_kontak">
                </div>
		
                <div class="form-group">
                <label>Alamat</label>
                <textarea rows="3" class="form-control" name="alamat" value=' . $selectedit2->alamat_prmj . '>' . $selectedit2->alamat_prmj . '</textarea>
                </div>      
                ...
                <div class="tab-pane" id="foto">
		<br>
                <input name="oldfoto" type="hidden" value="' . $selectedit2->foto_prmj . '" class="form-control">';
                    
                    if (empty($selectedit2->foto_prmj) OR ! file_exists($dirfoto . $selectedit2->foto_prmj)) {
                            echo '<img src="../images/foto_prmj/noimg.jpg" class="img-responsive img-thumbnail" style="width:300px; height:300px;">';
                        } else {
                            echo '
                            <img src="../images/foto_prmj/' . $selectedit2->foto_prmj . '" class="img-responsive img-thumbnail" style="width:300px; height:300px;">';
                        }
                        echo ' 
			
			<output id="gambar_nodin"></output>
			<input type="file" name="Filegambar" id="preview_gambar" class="btn btn-info" value=' . $selectedit2->foto_prmj . '/>';
                    ?>
                    <script>
	...
	<div class="panel-footer">
        <a href="detail-praremaja.php?detail=' . $_GET['edit'] . '" class="btn btn-warning btn-md"><span class="fa fa-angle-left" aria-hidden="true" title="kembali"></span> Kembali</a>
	...
	';
        if (isset($_POST['btn_ubah'])) {
            $input = new stdClass();
            $input->id = $_POST['tx_id_prmj'];
            $input->nm = antiinjection($_POST['tx_nm_prmj']);
            $input->klmn = $_POST['jk_kl'];
            $input->tmp_lhr = $_POST['tx_tmp_lhr'];
            $input->tgl_lhr = $_POST['tx_tgl_lhr'];
            $input->pdkn = $_POST['tx_pdkn'];
            $input->kls = $_POST['tx_kls'];
            $input->nm_sklh = $_POST['tx_nm_sklh'];
            $input->tpq_desa = $_POST['tx_tpq'];
            $input->bapak = $_POST['bapak'];
            $input->ibu = $_POST['ibu'];
            $input->hobi = $_POST['tx_hobi'];
            ...
            require_once "../db/database.php";
            $isi = "Update data_praremaja set id_prmj='" . $input->id . "' , nm_prmj='" . $input->nm . "' , j_klmn='" . $input->klmn . "',tmp_lhr='" . $input->tmp_lhr . "', tgl_lhr='" . $input->tgl_lhr . "',pdkn='" . $input->pdkn . "', kls='" . $input->kls . "',nm_sklh_p='" . $input->sklh . "',hobi='" . $input->hobi . "', tpq_desa='" . $input->tpq_desa . "', bapak='" . $input->bapak . "',bapak='" . $input->ibu . "', kontak_prmj='" . $input->kontak . "', alamat_prmj='" . $input->almt . "',foto_prmj='" . $input->foto . "'  WHERE id_prmj = '" . $input->id . "'";
            ...
            if ($sqltambah) {
                echo "<script type='text/javascript'>alert('Data berhasil diubah');window.location.href='detail-Praremaja.php?detail=$id';</script>";
            } else {
